...
export default function HotelManagerEarningsPage() {
    ...
    const load = async () => {
      try {
        ...
        const [summaryRes, txRes, eligibilityRes] = await Promise.all([
          paymentsApi.getHotelManagerEarningsSummary(),
        ...
        setError(e?.response?.data?.message || 'Failed to load earnings data')
      } finally {
        setLoading(false)
      }
    }
  ...
  const monthlyTrend = useMemo(() => {
    ...
    tx.forEach((t) => {
      ...
      grouped[key] = (grouped[key] || 0) + Math.max(0, parsePaisa(t.amount))
    })
  ...
  if (loading) {
    return <PageLoader message="Loading earnings dashboard..." variant="skeleton" />
  }
  ...
  const total = parsePaisa(summary?.earnings.fromTrips || summary?.earnings.total || '0')
